  ...
  """
    ...
    if region:
        btn = pyautogui.locateCenterOnScreen(
            img, confidence=confidence, minSearchTime=minSearch, region=region
        )
    else:
        btn = pyautogui.locateCenterOnScreen(
            img, confidence=confidence, minSearchTime=minSearch
        )
    if btn:
        if text:
            debug(text)
    ...
    while time.time() - start < get_secs(timeout):
        if region:
            box = pyautogui.locateOnScreen(
                img_path, confidence=confidence, region=region
            )
        else:
