...
def enhanced_screenshot(region=(0, 0, 1920, 1080)) -> Image.Image:
    # Create a fresh MSS instance for each call (thread-safe)
    with mss.mss() as sct:
        monitor = {
            "left": region[0],
            "top": region[1],
        ...
        # Convert MSS image to PIL safely
        img_np = np.array(
            sct_img, copy=True
        )  # make a copy to avoid thread-local issues
        img_rgb = img_np[:, :, :3][:, :, ::-1]  # BGR -> RGB
...
def enhance_image_for_ocr_2(pil_img: Image.Image, scale: float) -> np.ndarray:
    ...
    cv_img = cv2.resize(
        small, None, fx=scale * 2, fy=scale * 2, interpolation=cv2.INTER_CUBIC
    )
