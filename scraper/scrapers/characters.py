...
class CharacterScraper(BaseScraper):
    ...
    def start(self):
        ...
        items = [
            a
            for a in grid.find_elements(By.CSS_SELECTOR, "a.sc-73e3e686-1")
            if a.is_displayed()
        ]
    ...
    def _sort_by_name(self, driver):
        row = driver.find_element(
            By.XPATH, "//div[contains(@class, 'filters_sort_row')]"
        )
        first = row.find_element(By.XPATH, ".//select[1]")
        first.click()
        time.sleep(0.1)
        first.find_element(By.XPATH, ".//option[@value='name']").click()
        time.sleep(0.1)
        second = row.find_element(By.XPATH, ".//select[2]")
        second.click()
        time.sleep(0.1)
        second.find_element(By.XPATH, ".//option[@value='asc']").click()
        time.sleep(0.1)
