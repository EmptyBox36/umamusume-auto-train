...
def calculate_turn_number(date_string: str) -> int:
    ...
    if "debut" in date_string.lower():
        logging.warning(
            "Pre-Debut date detected in race data, this shouldn't happen. Returning turn 1."
        )
        return 1
    ...
    if year is None:
        ...
        logging.info(
            f"Year not found in mapping, using best match: {year_part} -> {best_year}"
        )
        year = best_year
    ...
    if month is None:
        ...
        logging.info(
            f"Month not found in mapping, using best match: {month_part} -> {best_month}"
        )
        month = best_month
...
def _format_date(header_text: str) -> str:
    ...
    m = re.search(
        r"(Junior|Classic|Senior)\s+Class\s+([A-Za-z]+),\s+(First|Second)\s+Half",
        header_text,
    )
    if not m:
        return header_text
...
class RaceScraper(BaseScraper):
    ...
    def _load_g1_sparks(self, driver) -> dict[str, list[str]]:
        ...
        rows = driver.find_elements(
            By.XPATH, "//div[contains(@class,'races_row_factors')]"
        )
        if not rows:
            # fallback: list container then item rows
            rows = driver.find_elements(
                By.XPATH,
                "//div[contains(@class,'races_race_list')]//div[contains(@class,'races_row')]",
            )

        for r in rows:
            # race name
            try:
                name_el = r.find_element(
                    By.XPATH,
                    ".//div[contains(@class,'races_name') or contains(@class,'races_desc_right')]/div[1]",
                )
            ...
            factor_box = None
            try:
                factor_box = r.find_element(
                    By.XPATH, ".//div[contains(@class,'races_factors')]"
                )
            except Exception:
            ...
            if factor_box:
                chips = factor_box.find_elements(
                    By.XPATH, ".//*[contains(@class,'factor') or self::span]"
                )
    ...
    def start(self):
        ...
        race_list = driver.find_element(
            By.XPATH, "//div[contains(@class,'races_race_list')]"
        )
        items = race_list.find_elements(
            By.XPATH, ".//div[contains(@class,'races_row')]"
        )
        ...
        links = [
            it.find_element(
                By.XPATH,
                ".//div[contains(@class,'races_ribbon')]//div[contains(@class,'utils_linkcolor')]",
            )
            for it in items
        ]
        ...
        for i, link in enumerate(links, start=1):
            ...
            dialog = driver.find_element(
                By.XPATH,
                "//div[@role='dialog']//div[contains(@class,'races_det_wrapper')]",
            )
            infobox = dialog.find_element(
                By.XPATH, ".//div[contains(@class,'races_det_infobox')]"
            )
            schedule = dialog.find_element(
                By.XPATH, ".//div[contains(@class,'races_det_schedule')]"
            )
            sched_items = schedule.find_elements(
                By.XPATH, ".//div[contains(@class,'races_schedule_item')]"
            )

            caps = infobox.find_elements(
                By.XPATH, ".//div[contains(@class,'races_det_item_caption')]"
            )
            vals = infobox.find_elements(
                By.XPATH, ".//div[contains(@class,'races_det_item__')]"
            )
            info = {c.text.strip(): v.text.strip() for c, v in zip(caps, vals)}

            race_name = dialog.find_element(
                By.XPATH, ".//div[contains(@class,'races_det_header')]"
            ).text.strip()
            race_name = normalize_race_name(race_name)
            date_header = (
                dialog.find_element(
                    By.XPATH, ".//div[contains(@class,'races_schedule_header')]"
                )
                .text.replace("\n", " ")
                .strip()
            )
            ...
            if len(sched_items) >= 2:
                ...
                if m1:
                    fans_required = int(m1.group(1))
                if m2:
                    fans_gained = int(m2.group(1))
            ...
            for h in headers:
                ...
                try:
                    date_el = h.find_element(
                        By.XPATH,
                        "following-sibling::div[contains(@class,'races_schedule_item')][1]/div[1]",
                    )
                ...
                if prev is None:
                    ...
                    if not any(
                        e["date"] == payload_copy["date"]
                        and e["distance"]["meters"]
                        == payload_copy["distance"]["meters"]
                        for e in prev
                    ):
                    ...
                    if not (
                        prev["date"] == payload_copy["date"]
                        and prev["distance"]["meters"]
                        == payload_copy["distance"]["meters"]
                    ):
            ...
            def _sort_bucket(bkt: dict, start_month: int = 1) -> dict:
                ...
                return {
                    name: (entries if len(entries) > 1 else entries[0])
                    for _, _, name, entries in packed
                }

            # close dialog
            driver.find_element(
                By.XPATH, "//div[contains(@class,'sc-f83b4a49-1')]"
            ).click()
            time.sleep(0.3)
        ...
        for year in ["Junior Year", "Classic Year", "Senior Year"]:
            if year in self.data:
                sorted_data[year] = _sort_bucket(
                    self.data[year], start_month=1
                )  # Jan..Dec
