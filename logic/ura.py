...
def ura_logic(results: dict) -> str:
    ...
    filtered = _filter_by_stat_caps(results, current_stats)
    ...
    if criteria_race is not None:
      info("[URA] Critical goal soon → Enter race.")
      prioritize_g1, race_name = decide_race_for_goal(year, turn, criteria, criteria_race)
    ...
    if _schedule_race() is True:
        return "exit"

    result, best_data = ura_training(filtered)

    if best_data and best_data["best_score"] < 2 and _summer_next_turn() :
        if _low_energy():
    ...
    if best_data and best_data["best_score"] >= 2:
    ...
    if best_data and best_data["best_score"] >= 1.1:
