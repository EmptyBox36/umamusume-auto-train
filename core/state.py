...
from core.ocr import extract_number, extract_text, extract_text_improved
...
CURRENT_STATS = {"spd": 0, "sta": 0, "pwr": 0, "guts": 0, "wit": 0}
LAST_VALID_STATS = {"spd": 0, "sta": 0, "pwr": 0, "guts": 0, "wit": 0}
CURRENT_YEAR = None
FORCE_REST = False
...
ENABLE_RACE_SCHEDULE = None
RUN_RACE_ON_POOR_TRAINING = None
VIRTUAL_TURN = None
...
def reload_config():
    ...
    global UNITY_TEAM_PREFERENCE, UNITY_SPIRIT_BURST_POSITION, ENABLE_RACE_SCHEDULE, RUN_RACE_ON_POOR_TRAINING, PURCHASED_SKILLS, SKILL_UMALATOR_URL
    ...
    ENABLE_RACE_SCHEDULE = config["enable_race_schedule"]
    RUN_RACE_ON_POOR_TRAINING = config["run_race_on_poor_training"]
    ...
    SKILL_UMALATOR_URL = config["skill"].get("umalator_url", "")
    ...
    SUMMER_PRIORITY_EFFECTS_LIST = {
        i: v for i, v in enumerate(config["summer_priority_weights"])
    }
...
def stat_state():
    ...
    if turn == "Goal":
        info("Race day!")
        stat_regions = {
            "spd": constants.SPD_RACE_STAT_REGION,
            "sta": constants.STA_RACE_STAT_REGION,
            "pwr": constants.PWR_RACE_STAT_REGION,
            "guts": constants.GUTS_RACE_STAT_REGION,
            "wit": constants.WIT_RACE_STAT_REGION,
        }
    else:
        stat_regions = {
    ...
    for stat, region in stat_regions.items():
        ...
        debug(f"Extracted {stat} stat: {val}")
        ...
        if "max" in text:
            debug(f"{stat.upper()} stat detected as MAX")
            result[stat] = cap
            continue

        # Case 3: Failed read → fallback to cap
        debug(f"Failed to read {stat} stat, defaulting to cap")
        result[stat] = cap
...
def check_support_card(threshold=0.8, target="none"):
    ...
    hint_matches = match_template(
        "assets/icons/support_hint.png", constants.SUPPORT_CARD_ICON_BBOX, threshold
    )
    white_flame_matches = match_template(
        "assets/unity_cup/white_flame.png", constants.SUPPORT_CARD_ICON_BBOX, threshold
    )
    blue_flame_matches = match_template(
        "assets/unity_cup/blue_flame.png", constants.SUPPORT_CARD_ICON_BBOX, threshold
    )
    ...
    count_result["total_non_maxed_support"] = count_result["total_supports"] - (
        count_result["total_friendship_levels"]["yellow"]
        + count_result["total_friendship_levels"]["max"]
    )
...
def check_energy_level(threshold=0.85):
    ...
    right_bar_match = match_template(
        "assets/ui/energy_bar_right_end_part.png", constants.ENERGY_BBOX, threshold
    )
    # longer energy bars get more round at the end
    if not right_bar_match:
        right_bar_match = match_template(
            "assets/ui/energy_bar_right_end_part_2.png",
            constants.ENERGY_BBOX,
            threshold,
        )
    ...
    if right_bar_match:
        ...
        MAX_ENERGY_BBOX = (
            x,
            top_bottom_middle_pixel,
            x + energy_bar_length,
            top_bottom_middle_pixel + 1,
        )
...
def check_status_effects():
    ...
    matches = [
        k for k in BAD_STATUS_EFFECTS if k.lower().replace(" ", "") in normalized_text
    ]
